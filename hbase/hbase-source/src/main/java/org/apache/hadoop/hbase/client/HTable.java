...
public class HTable implements HTableInterface, RegionLocator {
     ...
     * @return {@code true} if table is online.
     * @throws IOException if a remote or network exception occurs
     * @deprecated use {@link HBaseAdmin#isTableEnabled(TableName tableName)}
     */
    @Deprecated
     ...
     * {@link #batchCallback(List, Object[], Callback)}
     * instead.
     */
    @Deprecated
    @Override
    public <R> Object[] batchCallback(
            final List<? extends Row> actions, final Callback<R> callback) throws IOException,
     ...
     * 客户端的put请求调用getBufferedMutator().mutate(put)，
     *
     * @param put
     * @throws IOException
     */
    ...
    public <R> void processBatchCallback(
            final List<? extends Row> list, final Object[] results, final Callback<R> callback)
            throws IOException, InterruptedException {
    ...
    public <T extends Service, R> Map<byte[], R> coprocessorService(final Class<T> service,
        ...
        coprocessorService(service, startKey, endKey, callable, new Callback<R>() {
            @Override
    ...
    public <T extends Service, R> void coprocessorService(final Class<T> service,
        ...
        for (Entry<byte[], Future<R>> e : futures.entrySet()) {
            try {
                e.getValue().get();

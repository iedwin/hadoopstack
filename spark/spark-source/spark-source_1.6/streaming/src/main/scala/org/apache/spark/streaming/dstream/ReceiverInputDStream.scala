...
/**
  *
  * 具体为什么我们说一个block会作为一个partition来进行计算，这一点可以看一下ReceiverInputDStream类的compute方法，
  * 该方法调用了createBlockRDD方法来创建基于Receiver模式的RDD。
  * 在该方法中可以看到最终封装的RDD为BlockRDD或者WriteAheadLogBackedBlockRDD。
  *
  * BlockRDD类中getPartitions方法是说将这个batch的blocks作为partitions。
  * Compute方法则按照入参BlockRDDPartition的blockId，从blockManager中获取该block作为partition的数据。
  * getPreferredLocations则是将BlockRDDPartition所在的host作为partition的首选位置。
  *
  *

...
class AsyncProcess {
     ...
     * <p>
     * <p>
     * 循环遍历参数writeAsyncBuffer中的每一行，通过connection.locateRegion函数找到其在集群的位置loc，将该位置与操作action一起绑定在变量actionByServer中。
     */
     ...
     * <p>
     * <p>
     * 这里的region定位是由ClusterConnection类型的变量connection完成的，进入其locateRegion方法可以看出，
     * <p>
     * 如果客户端有缓存，则直接从缓存读取，否则从META表中读出了region所处的位置，并缓存此次的读取结果。返回的结果是RegionLocations类型的变量。
     * <p>
     * <p>
     * actionByServer是一个Map<ServerName,MulteAction<Row>>类型的变量，
     * <p>
     * 从该变量的类型定义可以看出，其将用户的一批写请求中，写入regionserver地址相同的动作归类到一起
     *
    ...
    protected class AsyncRequestFutureImpl<CResult> implements AsyncRequestFuture {
         ...
         *
         *
         * 其将用户的action请求通过getNewMultiActionRunnable、SingleServerRequestRunnable层层调用最终落到了hbase的RPC框架中，
         *
         * 每个用户请求包装成包装MultiServerCallable对象，其是一个Runnable对象，在该对象中用户请求与服务端建立起RPC联系。
         *
         * 所有的runnable对象最终交到AsyncProcess对象的内部线程池中处理执行。
         *

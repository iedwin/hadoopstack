...
/**
  * Configuration for a Spark application. Used to set various(各种) Spark parameters as key-value pairs(对).
  *
  * Most of the time, you would create a SparkConf object with `new SparkConf()`, which will load
  * values from any `spark.*` Java system properties set in your application as well. In this case,
  * parameters you set directly on the `SparkConf` object take priority（优先） over system properties.
  *
  * For unit tests, you can also call `new SparkConf(false)` to skip loading external settings and
  * get the same configuration no matter what the system properties are.
  *
  * All setter methods in this class support chaining（链接）. For example, you can write
  * `new SparkConf().setMaster("local").setAppName("My app")`.
  *
  *
  * 注意，一旦sparkconf对象传递到spark，它是克隆的，用户不能再进行修改。spark不支持在运行时修改配置。
...
class SparkConf(loadDefaults: Boolean) extends Cloneable with Logging {
    ...
    /** Set JAR files to distribute(分发) to the cluster. */

...
public class BufferedMutatorImpl implements BufferedMutator {
    ...
    /**
     * 提交的put操作放入到列表writeAsyncBuffer中，当buffer中的数据超过规定值时，由后台进程进行提交。
     */
    private void doMutate(Mutation m) throws InterruptedIOException,
        ...
        while (currentWriteBufferSize > writeBufferSize) {
            backgroundFlushCommits(false);//【重要】
        }
    }
    ...
    private void backgroundFlushCommits(boolean synchronous) throws InterruptedIOException,
        ...
        try {
            if (!synchronous) {
                ap.submit(tableName, writeAsyncBuffer, true, null, false); //异步后台提交
                if (ap.hasError()) {
            ...
            if (synchronous || ap.hasError()) {
                while (!writeAsyncBuffer.isEmpty()) {                     //同步后台提交

                    //可以发现无论异步提交还是同步提交，实际的提交动作是由AsyncProcess ap执行的

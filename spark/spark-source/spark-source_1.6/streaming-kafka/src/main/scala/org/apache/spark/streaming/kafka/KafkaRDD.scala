...
/**
  ...
  *
  *
  * 通过kafkaRDD这个类的阅读我们可以看出，接收数据是以partition的leader为维度做分布式的，
  * 这样做可以保证这个host上是有我要消费的数据的，能够实现数据本地化。
  * 这样后续在搭建spark集群时，可以考虑在spark集群上搭建一个kafka集群专门给spark集群使用
  *
...
R: ClassTag] private[spark](
    ...
    /**
      * KafkaRDD的partition个数就是topic的partition个数之和
      *
      * @return
      */
    ...
    /**
      *
      * partition的首选location就是该topic的某个partition的leader所在的host。
      * 这是很合理的，因为leader上的数据正常情况下是最新的而且是最准确的。
      * 而follower的数据往往还需要从leader上做同步，并且一旦同步出现较大的落后，还会从in-sync列表中移除
      *
      * @param thePart
      * @return
      */
    ...
    private class KafkaRDDIterator(
        ...
        //【重要】
        //spark streaming的kafka低阶API是每一个partition起一个consumer来消费数据的
        ...
        /**
          *
          * 根据起止offset消费kafka某topic某partition数据的代码。
          *
          * @return
          */
